...
const generateTpl = (lines = [], route = {}) => {
    if ("/module/page" === route.uri) {
         ...
         */
        lines.push(`{routine("/ui/:module/:page",Container["${route.layout}"],Component["${route.page}"])}`);
    } else if ($typePages.contains(route.uri)) {
        /*
         * 字典页面
         */
        lines.push(`{routine("${route.uri}/:type",Container["${route.layout}"],Component["${route.page}"])}`);
    } else {
        lines.push(`{routine("${route.uri}",Container["${route.layout}"],Component["${route.page}"])}`);
    }
    /*
     * 首页
     */
    if ("/login/index" === route.uri) {
        lines.push(`{routine("/",Container["${route.layout}"],Component["${route.page}"])}`);
    }
};
