...
const $opAddRank = (reference: any) => Ux.ai2Event(reference, (values) => {
    console.info("归类添加", values);
    const {key} = reference.props.$datum;
    Ux.microPost(Cv.App.Modal, "/api/categories/:mid", {mid: key, $body: Ux.Uson.create(values).add("type", "model.rank").to()})
