...
class DataArray implements DataContainer {
    ...
    __type(): String {
        return __Zn.Env.E_TYPE.DATA_ARRAY;
    }

    setValue(data: any) {
        this.data = __Zn.wayO2S(data); // ?SON.stringify(data);
        this.ready = !!data;
    ...
    filter(fnFilter: Function) {
        const data = JSON.parse(this.data);
        // @ts-ignore
        if (__Zn.isFunction(fnFilter)) {
            const result = data.filter(fnFilter);
            this.data = __Zn.wayO2S(result);
        } else { // @ts-ignore
            if (__Zn.isObject(fnFilter)) {
                const result = __Zn.elementFind(data, fnFilter);
                this.data = __Zn.wayO2S(result);
            }
        }
        return this;
    ...
    saveElement(element: any, idField: any = "key") {
        if (element) {
            if (!element[idField]) {
                element[idField] = __Zn.randomUUID();
            }
            // 元素信息处理
            if (!this.data) {
                this.setValue([]);
    ...
    getElement(key: string, idField: any = "key") {
        ...
        if (this.data && key) {
            ...
            // @ts-ignore
            if (__Zn.isArray(dataArray) && 1 === dataArray.length) {
                result = dataArray[0];
            }
        }
        return result;
