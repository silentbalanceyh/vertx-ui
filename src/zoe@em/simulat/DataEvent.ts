...
class DataEvent {
    ...
    clone(definition) {
        /* 拷贝专用方法 */
        return new DataEvent(__Zn.clone(this.data))
            .start(__Zn.clone(this._input))
            .bind(this._ref)
            .metadata(definition)
            .config(this._config)
            .previous(__Zn.clone(this._previous))
            .response(__Zn.clone(this._response))
    }
     ...
     */
    next(response) {
        __Zn.dgDebug(response, `[ ${this._id} ] ${this._response.length + 1}. 输出 --->`, `#969696`);
        this._previous = response;
        this._response.push(response);
        return __Zn.promise(this);
    }
    ...
    getData() {
        __Zn.dgDebug(this._input, `[ ${this._id} ] ${this._response.length + 1}. ---> 输入`, `#6C7B8B`);
        return this.data;
    }
