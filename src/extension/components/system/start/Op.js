    ...
    if (!source.jdbcUrl) {
        ...
        Ux.ajaxPost(`/api/connect`, source).then(response => {
            ...
            if ("FAILURE" === result) {
                ...
                Ux.of(result).in({$connected: false}).done();
                // reference.?etState({$connected: false});
            } else {
                // 数据库连接成功
                Ux.showDialog(reference, "connected", () => {
                    Ux.rdxSubmitting(reference, false);
                    Ux.of(result).in({$connected: true}).done();
                    // reference.?etState({$connected: true});
                })
            }
        });
    }
...
const _nextPhase = (reference, values) => new Promise(resolve => {
    ...
    if ($data && values) {
        ...
        Ux.of(reference).in({$data}).handle(() => resolve($data));
...
const _nextStep = (reference) => ($data) => {
    ...
    // 设置激活的tab和选中项
    Ux.of(ref).in({$data, $activeKey}).done();
    // ref.?etState({$data, $activeKey});
};
    ...
    Ux.ajaxGet("/api/app/pre-init", {name: $data.name})
        .then(response => {
            ...
            Ux.of(ref).in({
                $data,
                $activeKey,
                $initData: Ux.clone(response)
            }).done();
            // ref.?etState({$data, $activeKey, $initData: Ux.clone(response)});
        });
};
