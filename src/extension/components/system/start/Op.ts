...
const $opConnect = (reference: any) => Ux.ai2Event(reference, (values: any) => {
    const {source = {}} = values;
    if (!source.jdbcUrl) {
...
const _nextPhase = (reference: any, values: any) => {
...
const _verifySource = (reference: any) => {
    const {$connected = false} = reference.state;
    return !$connected;
};
const _nextStep = (reference: any) => ($data: any) => {
...
const _nextStep2 = (reference: any) => ($formData: any = {}) => {
    // 读取上级引用，并且改变上级的Tab页处理
    const ref = Ux.onReference(reference, 1);
    const $activeKey = _nextTab(ref);
...
const bindImport = (reference: any) => (event: any) => {
    event.preventDefault();
    const ref = Ux.onReference(reference, 1);
    const {$data = {}} = ref.state;
    if (!Ux.isEmpty($data)) {
...
export default {
     ...
     * 保存数据源按钮
     */
    $opNext1: (reference: any) => Ux.ai2Event(reference, (values) => Ux.rxOp(reference)
        .success(() => _nextPhase(reference, values))
        .to(_nextStep(reference))),
    // 第二页的按钮
    $opNext2: (reference: any) => Ux.ai2Event(reference, (values) => Ux.rxOp(reference)
