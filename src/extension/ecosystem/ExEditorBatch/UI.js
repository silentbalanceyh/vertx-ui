...
const yiOp = (reference, config = {}, $control = {}) => {
    ...
    // Fix: https://gitee.com/silentbalanceyh/vertx-zero-scaffold/issues/I6VREB
    column.width = 120;
    ...
    column.render = (text, record, index) => {
        ...
        options = options.filter(item => columns.includes(item.key));
        // ------------- 属性构造 --------------
        return Ux.aiAddRemove(reference, {
...
const componentInit = (reference) => {
    ...
    // reference.?etState({$columns, $control, $data, $combine, $ready: true})
};
