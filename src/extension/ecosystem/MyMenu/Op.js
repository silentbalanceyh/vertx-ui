...
const onSwitch = (reference) => ($admin) => {
    ...
    if ($changed) {
        Ux.sexDialog(reference, "unsave", {
            ok: () => reference.setState({$admin, $changed: undefined})
        })
    } else {
        reference.setState({$admin, $changed: undefined});
    }
}
...
const onSave = (reference) => (event) => {
    ...
    request.menus = menus;
    Ux.ajaxPost("/api/my/menu/save", request)
        .then(response => Ux.sexDialog(reference, "saved", () => {
            reference.setState({
                $data: response,
                $submitting: false,
                $changed: undefined
...
const toolkit = (reference, item = {}) => ({
    rxLeft: (event) => {
        ...
        reference.setState({$data: data, $changed: Ux.randomString(8)})
    },
    rxRight: (event) => {
        Ux.prevent(event);
        const {$data = []} = reference.state;
