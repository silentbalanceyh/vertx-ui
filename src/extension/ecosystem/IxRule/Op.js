...
const onData = (reference, key, selected = []) => {
    ...
    Ux.of(reference).in({data: $data}).handle(() => {
...
const onRemove = (reference, key, removed) => {
    ...
    // reference.?etState({data: $data, $selectedFields});
    // onChange(reference, $data);
}
export default {
    /* 选中某个标识规则 */
    onRowSelected: (reference) => ($selectedKeys) => {
        ...
        // reference.?etState({$selectedKeys, $selectedFields});
    },
    onChecked: (reference) => ($selectedFields = []) => {
        /* 选中标识规则（追加）*/
        // reference.?etState({$selectedFields})
        Ux.of(reference).in({
            $selectedFields
        }).done()
    },
    ...
    onRule: (reference, field) => (each) => {
        const {data = {}} = reference.state;
        data[field] = each;
        Ux.of(reference).in({data}).handle(() => {
