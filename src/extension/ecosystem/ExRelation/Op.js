...
const onChange = (reference, key) => (keys = []) => {
    ...
    if ("up" === key) {
        of.in({$selectedUp: keys});
        // reference.?etState({$selectedUp: keys});
    } else {
        of.in({$selectedDown: keys});
        // reference.?etState({$selectedDown: keys});
    }
    of.done();
};
...
const yoTable = (reference, table = {}, key, combine) => {
    ...
    $table.className = "ux_table r_table";
