...
import On from './O.event.cmd.on';
import {DiActions, DiMap, DiNames} from "./I.injections";
/*
...
const generateFn = (gEvent, name) => {
    return (item) => {
        ...
        if (reference) {
            // 注入 default 函数
            let params: any = {};
            const executorDefault = DiActions[name];
            if (Abs.isFunction(executorDefault)) {
...
class GCommand {
    ...
    initialize() {
         ...
         */
        graph.on('node:mouseenter', On.combineFn([
            On.portShowFn(gEvent),
            On.nodeAddonFn(gEvent)
        ]));
        graph.on('node:mouseleave', On.combineFn([
            On.portShowFn(gEvent, false),
            On.toolRemoveFn(gEvent)
        ]));
        graph.on('blank:click', On.portShowFn(gEvent, false));
         ...
         */
        graph.on('edge:mouseenter', On.edgeAddonFn(gEvent));
        graph.on('edge:mouseleave', On.toolRemoveFn(gEvent));
        graph.on('edge:change:target', On.portShowFn(gEvent));
        graph.on('edge:change:source', On.portShowFn(gEvent));
         ...
         * 2）再添加
         */
        graph.on('edge:connected', On.connectedFn(gEvent));

        graph.on('node:added', On.nodeAddFn(gEvent));
        graph.on('node:removed', On.nodeRemoveFn(gEvent));
        graph.on('node:click', On.nodeClickFn(gEvent));
        graph.on('node:dblclick', On.nodeClickFn(gEvent, true));
    }
