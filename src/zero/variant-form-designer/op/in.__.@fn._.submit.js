...
export default (normalized = {}, data = {}, reference) => {
    /* 是否隐藏 */
    normalized.isHidden = __Zn.valuePath(data, "hidden");
    const {$form = {}} = reference.props;
    const {actions = {}} = $form;
    /* Extension */
    const extension = __Zn.valuePath(data, "optionJsx.extension");
    if (__Zn.isArray(extension)) {
        const resetFound = __Zn.elementUnique(extension, "event", "RESET");
        if (resetFound) {
