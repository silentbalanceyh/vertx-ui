...
export default (targetRef) => (fromItem, toItem) => {
    ...
    const sourceRef = __Zn.onReference(source, 1);
    /*
    * 1. 读取源 $rows
    * 2. 读取目标 $rows
    * */
    if (sourceRef && targetRef) {
        ...
        sourceCells = __Zn.clone(sourceCells);
        let targetCells = targetRef.props.data;
        targetCells = __Zn.clone(targetCells);
        /*
         * fromItem 拖拽到 toItem
         */
        if (fromData.rowIndex === toItem.rowIndex) {
             ...
             * 2）交换 span
             */
            const sourceItem = __Zn.clone(fromData);
            sourceItem.cellIndex = toItem.cellIndex;
            sourceItem.span = toItem.span;

            const targetItem = __Zn.clone(toItem);
            ...
            __Zn.fn(sourceRef).rxRowConfig(results);
        }
    }
}