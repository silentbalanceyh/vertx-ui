 ...
 * 过滤格式化：projection
 */
const irViewV = (query: any = {}) => {
    let projection = [];
    if (Array.isArray(query.projection)) {
        projection = __Zn.clone(query.projection);
    }
    return projection;
};
/*
...
const irSorter = (query: any = {}) => {
    ...
    if (query.sorter) {
        if (Array.isArray(query.sorter)) {
            sorterData = __Zn.clone(query.sorter);
        } else {
...
const irCriteria = (query: any) => {
    ...
    if (query.criteria) {
        if ("string" === typeof query.criteria) {
            ...
            // 3. 本身就是一个对象，直接传入
            criteria = __Zn.clone(query.criteria);
        }
    }
    return criteria;
};
