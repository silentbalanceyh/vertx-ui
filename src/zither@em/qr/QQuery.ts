...
class QQuery {
    ...
    constructor(query = {}, reference) {
        ...
        this.query.projection = Qt.irViewV(query);
    ...
    to() {
        const $query = __Zn.clone(this.query);
        $query.criteria = this.forest.to();
        __Zn.dgQr($query.criteria, "[ Qr ] 森林分析最终条件 = ");
        return $query;
    }

    trace() {
        const queryData = this.forest.to();
        __Zn.dgQr(queryData, "[ Qr ] （Debug）森林分析条件 = ");
    }
}
