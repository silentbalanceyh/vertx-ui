...
const _generateOn = (reference, inherit = {}, names = []) => {
    names.forEach(name => {
        const st1 = `on${name}`;
        // 从 state中提取
        let executor;
        if (reference.state) {
            executor = reference.state[st1];
        }
        if (!executor || !isFunction(executor)) {
            executor = reference.props[st1];
        }
        if (!executor || !isFunction(executor)) {
            const nd2 = `on2${name}`;
            let executor2;
            if (reference.state) {
                executor2 = reference.state[nd2];
            }
            if (!executor2 || !isFunction(executor2)) {
                executor2 = reference.props[nd2];
            }
            if (isFunction(executor2)) {
