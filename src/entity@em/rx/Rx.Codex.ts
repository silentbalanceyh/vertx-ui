...
class Rule {
    ...
    condition(record: any): any {
        return __Zn.isFunction(this._fnCond) ? this._fnCond(record) : false;
    }

    done(record: any): any {
        return __Zn.isFunction(this._fnDone) ? this._fnDone(record) : record;
    }
}
...
class Codex {
    ...
    bind(input: any): Codex {
        ...
        if ("string" === typeof input) {
            const config = __Zn.fromHoc(this._reference, input);
            if (__Zn.isArray(config)) {
                rules = __Zn.clone(config);
            }
        } else if (__Zn.isArray(input)) {
            rules = __Zn.clone(input);
        }
